...
- solution(s) to solve the problem of:
  ...
  - implements tape deck logic on a tape deck rendered into the DOM object
  - supports multiple instantiation, ie. can co-exist with other instances of the same (and the other, if appropriate) solution
- write some unit tests for the solution
...
- node/npm (**NOTE:** due to the limitations of some pinned library versions you will need to use node version 1.8 or earlier to run the unit tests on the command line)
...
You probably do not need to run a web server to serve up the project, everything *should* operate correctly when loaded in a web browser directly from the filesystem. YMMV depending on your environment.
...
- code to allow multiple instantiations, handling multiple tapedecks on the same page without interference
- unit tests
  - no not our unit tests, yours!
  - we would like you to write a few unit tests to "prove the implementation works" or rather, demonstrate that you can do basic unit testing
  - complete code/logic coverage is not necessary, don't feel obligated to write more than 4 or 5 representative test cases (but you can if you want!)
...
You can run the unit tests directly in your web browser by opening `test/test-both/index.html`.
...
- When the pause button is pressed
  ...
  - ie. it's a toggle
- The record button
  - May only be pressed while the Play button is depressed